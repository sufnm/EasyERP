...
export default function ChartOfAccountsPage({ setActivePage }) {
  ...
  const [headerClass, setHeaderClass] = useState('All');
  const [openDropdownId, setOpenDropdownId] = useState(null);
  const dropdownRef = React.useRef(null);

  useEffect(() => {
    setLoading(true);
    fetch(API_ENDPOINTS.ACCOUNT_ALL)
      .then(res => res.json())
      .then(data => {
        setAccounts(data.map(acc => ({
          ...
          name: acc.ACC_NAME,
          class: acc.ACC_CLASS,
          subClass: String(acc.LEVEL2_NO || ''),
          headerClass: String(acc.LEVEL3_NO || ''),
          status: 'Active'
        })));
        setLoading(false);
      })
  ...
  // Fetch sub-classes when accountClass changes
  useEffect(() => {
    if (accountClass === 'All') {
      setSubClasses([]);
      setSubClass('All');
      ...
      fetch(API_ENDPOINTS.ACCOUNT_SUBCLASSES(accountClass))
        .then(res => res.json())
        .then(data => {
          setSubClasses(data);
        })
  ...
  const filteredAccounts = accounts.filter(acc =>
    ...
    (accountClass === 'All' || String(acc.class) === String(accountClass)) &&
    (subClass === 'All' || String(acc.subClass) === String(subClass)) &&
    (headerClass === 'All' || String(acc.headerClass) === String(headerClass))
  );
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col flex-1 h-full relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-2">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">View and manage your complete chart of accounts.</p>
          </div>
          <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:w-auto">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    {classes.map(cls => (
                      <option key={cls.acc_class_code} value={cls.acc_class_code}>
                        {cls.acc_class_name}
                      </option>
                    ))}
                  </select>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    disabled={accountClass === 'All'}
                    className={`bg-white dark:bg-zinc-800 border border-border rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all dark:text-zinc-100 min-w-[140px] ${accountClass === 'All' ? 'opacity-50 cursor-not-allowed bg-zinc-50 dark:bg-zinc-900' : ''
                      }`}
                  >
                    <option value="All">All Sub Classes</option>
                    {subClasses.map(sub => (
                      <option key={sub.acc_no} value={sub.acc_no}>
                        {sub.acc_name}
                      </option>
                    ))}
                  </select>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] uppercase font-black text-zinc-400 dark:text-zinc-500 px-1">Header Class</label>
                  <select
                    value={headerClass}
                  ...
                  >
                    <option value="All">All Header Classes</option>
