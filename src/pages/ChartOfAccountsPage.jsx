...
export default function ChartOfAccountsPage({ setActivePage, params = {} }) {
  ...
  // Initialize from params if coming back
  const [accountClass, setAccountClass] = useState(params.accClass || 'All');
  ...
  useEffect(() => {
    setLoading(true);
    const levelMap = { '1': 2, '2': 3, '3': 4 };
    const targetLevel = levelMap[detailType] || 4;

    // Build URL based on filters
    let url;
    if (accountClass === 'All' || detailType === '0') {
      url = `${API_ENDPOINTS.ACCOUNT_CACHE}?level=${detailType === '0' ? 0 : targetLevel}`;
    } else {
  ...
  const filteredHeaderTypes = headerTypes.filter(h => {
    ...
    if (code === '0') return true; // Always show Master List
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col flex-1 h-full relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-2">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">View and manage your complete chart of accounts hierarchy.</p>
          </div>
          <button
            onClick={() => {
              const firstItem = filteredAccounts[0];
              setActivePage('create-account', { 
                accClass: accountClass !== 'All' ? accountClass : (firstItem?.class || '1'), 
            ...
            disabled={(() => {
              if (detailType === '0') return true; // Cannot create from Master List
              if (accountClass === 'All') return true;
              if (detailType === '1') return false; // Level 2 creation only needs Class
              if (detailType === '2') return subClass === 'All'; // Level 3 creation needs Sub Class
              if (detailType === '3') return subClass === 'All' || headerAcc === 'All'; // Level 4 creation needs Header
              return true;
            })()}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold transition-all text-sm ${
              (() => {
                if (detailType === '0') return true;
                if (accountClass === 'All') return true;
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 w-full lg:w-auto">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    {classes.map(cls => (
                      <option key={cls.acc_class_code} value={cls.acc_class_code}>{cls.acc_class_name}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select 
                    ...
                    className="bg-white dark:bg-zinc-800 border border-border rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all dark:text-zinc-100 min-w-[140px]"
                  >
                    {filteredHeaderTypes.map(h => (
                      <option key={h.header_code} value={h.header_code}>{h.header_name}</option>
                    ))}
                  </select>
                </div>
              </div>
          ...
          <div className="overflow-x-auto h-full">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                {loading ? (
                  <tr><td colSpan="8" className="p-8 text-center text-zinc-500">Loading hierarchy...</td></tr>
                  ...
                  filteredAccounts.map((account) => (
                    <tr key={account.id} className="border-b border-border hover:bg-indigo-500/5 transition-colors group">
                      ...
                      <td className="p-4"><div className="font-bold text-zinc-900 dark:text-zinc-200">{account.name}</div></td>
                  ...
                  <tr><td colSpan="8" className="p-8 text-center text-zinc-500 text-xs italic">No accounts found for the selected level.</td></tr>
                )}
              </tbody>
            </table>
          </div>
