    ...
    return (
      <div className="flex h-full min-h-[400px] items-center justify-center">
        <div className="flex flex-col items-center gap-4 p-8 bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-800 max-w-md text-center transition-colors duration-300">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 font-semibold text-xs mt-1 leading-relaxed">You do not have permission to view Supplier Payable. Please contact your system administrator.</p>
          <button 
      ...
      <div className="max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full pl-16 pr-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-black text-zinc-900 dark:text-zinc-100 outline-none focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all shadow-inner"
                      placeholder="0.00"
                      required
                    />
                  </div>
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-5 transition-colors duration-300">
              ...
              {!selectedInvoice ? (
                <div className="flex flex-col items-center justify-center py-10 px-4 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/20 text-center">
                  <div className="h-10 w-10 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 dark:text-zinc-500 mb-3">
                    <Search size={18} />
                  </div>
                  <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">{t('noInvoiceSelected')}</p>
                  <p className="text-[9px] text-zinc-400/80 dark:text-zinc-500/80 mt-1">Select an invoice from the list to display details and payment timeline.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="p-3.5 bg-rose-500/5 dark:bg-rose-500/10 rounded-xl border border-rose-500/10 dark:border-rose-500/20">
                    <p className="text-[9px] font-black text-rose-600 dark:text-rose-400 uppercase tracking-wider">Supplier</p>
                    <p className="text-sm font-black text-zinc-800 dark:text-zinc-100 mt-0.5 truncate">{selectedInvoice.ENAME}</p>
                  </div>
                  
                  <div className="space-y-2.5">
                    <div className="flex justify-between items-center border-b border-dashed border-zinc-100 dark:border-zinc-800/80 pb-2">
                      <span className="text-[9px] text-zinc-400 dark:text-zinc-500 font-black uppercase tracking-wider">{t('invoiceNo')}</span>
                      <span className="text-xs font-black text-zinc-800 dark:text-zinc-200">{selectedInvoice.INVOICE_NO}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-dashed border-zinc-100 dark:border-zinc-800/80 pb-2">
                      <span className="text-[9px] text-zinc-400 dark:text-zinc-500 font-black uppercase tracking-wider">Net Amount</span>
                      <span className="text-xs font-bold text-zinc-800 dark:text-zinc-200">SAR {Number(selectedInvoice.NET_AMOUNT || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-dashed border-zinc-100 dark:border-zinc-800/80 pb-2">
                      <span className="text-[9px] text-zinc-400 dark:text-zinc-500 font-black uppercase tracking-wider">Paid Amount</span>
                      <span className="text-xs font-bold text-zinc-800 dark:text-zinc-200">SAR {Number((selectedInvoice.CASH_PAID || 0) + (selectedInvoice.OTHER_PAID || 0)).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-dashed border-zinc-100 dark:border-zinc-800/80 pb-2">
                      <span className="text-[9px] text-zinc-400 dark:text-zinc-500 font-black uppercase tracking-wider">{t('balanceDue')}</span>
                      <span className="text-xs font-black text-rose-600 dark:text-rose-400">SAR {Number(selectedInvoice.BALANCE_AMT || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                    </div>
                  </div>
                  ...
                  {savedTransactions.filter(tx => tx.DOC_NO === selectedInvoice.INVOICE_NO).length > 0 && (
                    <div className="mt-4 pt-3.5 border-t border-zinc-100 dark:border-zinc-800/80">
                      <h4 className="text-[9px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                        <CheckCircle2 size={12} className="text-rose-500 dark:text-rose-400" />
                      ...
                      <div className="space-y-2 max-h-[150px] overflow-y-auto pr-1">
                        {savedTransactions
                          ...
                          .map((tx, idx) => (
                            <div key={idx} className="flex justify-between items-center p-2.5 bg-rose-500/5 dark:bg-rose-500/10 rounded-xl border border-rose-500/10 dark:border-rose-500/20">
                              <div className="flex flex-col">
                                <span className="text-[9px] font-black text-zinc-700 dark:text-zinc-200">{new Date(tx.ENTRY_DATE).toLocaleDateString()}</span>
                                <span className="text-[8px] text-zinc-400 dark:text-zinc-500 truncate w-24">{tx['TO ACC']}</span>
                              </div>
                              <span className="text-[10px] font-black text-rose-600 dark:text-rose-400">SAR {Number(tx.PAY_AMOUNT).toFixed(2)}</span>
                            </div>
                          ))}
                      </div>
                    </div>
                  ...
                  {formData.paymentAmount && Number(formData.paymentAmount) > Number(selectedInvoice.BALANCE_AMT) && (
                    <div className="mt-3 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-xl flex items-start gap-2.5 animate-pulse">
                      <ShieldAlert size={14} className="text-amber-600 dark:text-amber-400 mt-0.5 flex-shrink-0" />
                      <p className="text-[9px] font-bold text-amber-700 dark:text-amber-400 leading-normal">
                        WARNING: Payment amount (SAR {formData.paymentAmount}) exceeds the outstanding balance (SAR {Number(selectedInvoice.BALANCE_AMT).toFixed(2)}).
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
          <div className="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/40 dark:bg-zinc-900/40 flex items-center justify-between">
            <h3 className="text-xs font-black text-zinc-800 dark:text-zinc-200 uppercase tracking-widest flex items-center gap-2">
              <CheckCircle2 size={14} className="text-rose-500 dark:text-rose-400" />
            ...
            <table className={`w-full ${language === 'ar' ? 'text-right' : 'text-left'} border-collapse`}>
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-100 dark:border-zinc-800/80">
                  ...
                  <th className="px-6 py-3 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Paid To</th>
                  <th className="px-6 py-3 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Paid From</th>
                  ...
                  <th className="px-6 py-3 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Type</th>
              ...
              <tbody className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                {savedTransactions.map(tx => (
                  <tr key={tx.ID} className="border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50/50 dark:hover:bg-zinc-950/40 transition-colors duration-200">
                    ...
                    <td className="px-6 py-3.5 font-black text-zinc-900 dark:text-zinc-100">{tx.DOC_NO}</td>
                    <td className="px-6 py-3.5">
                      <div className="font-bold text-zinc-800 dark:text-zinc-200">{tx['TO ACC']}</div>
                    ...
                    <td className="px-6 py-3.5">
                      <div className="font-bold text-zinc-800 dark:text-zinc-200">{tx.FROM_ACC}</div>
                    ...
                    <td className="px-6 py-3.5">
                      {tx.RETURN_INVOICE ? (
                        <span className="px-2.5 py-1 bg-rose-100 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 rounded-full text-[9px] font-black uppercase tracking-wider border border-rose-200/40 dark:border-rose-900/40">Return</span>
                      ) : (
                        <span className="px-2.5 py-1 bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-wider border border-emerald-200/40 dark:border-emerald-900/40">Normal</span>
                      )}
                    </td>
                ...
                {savedTransactions.length === 0 && (
                  <tr>
                    <td colSpan="9" className="px-6 py-12 text-center text-zinc-400 dark:text-zinc-500 italic font-semibold">No transactions found for this period</td>
                  </tr>
                )}
              </tbody>
            </table>
