...
export default function PurchaseAccountsPage({ setActivePage }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(API_ENDPOINTS.PURCHASE_LIST)
      .then(res => {
      ...
      .then(data => {
        const mappedData = data.map(acc => ({
          ...
          name: acc.acc_name || acc.ACC_NAME || 'Unnamed Purchase Account',
    ...
    <div className="flex flex-col h-full p-6 animate-in fade-in zoom-in-95 duration-300">
      <div className="max-w-7xl mx-auto w-full flex flex-col flex-1 h-full relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-2">
          <div>
            <h1 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase">Purchase Accounts</h1>
            <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">Manage and track your purchase-related expense accounts.</p>
          </div>
          <button 
            onClick={() => setActivePage('customer-account-form', { type: 'Purchase' })}
          ...
          <div className="overflow-x-auto h-full">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                {loading ? (
                   <tr><td colSpan="8" className="p-8 text-center text-zinc-500 italic">Loading purchase accounts...</td></tr>
                  ...
                  filteredAccounts.map((account) => (
                    <tr 
                      ...
                      onClick={() => setActivePage('customer-account-form', { id: account.id, type: 'Purchase' })}
                    >
                        ...
                        {openDropdownId === account.id && (
                          ...
                          >
                            <button
                              onClick={(e) => {
                                 e.stopPropagation();
                                 setActivePage('customer-account-form', { id: account.id, type: 'Purchase' });
                              }}
                  ...
                  <tr><td colSpan="8" className="p-8 text-center text-zinc-500 italic">No purchase accounts found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
