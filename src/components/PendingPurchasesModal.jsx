...
import { X, Clock, User, DollarSign, ArrowRight, Package } from 'lucide-react';

export default function PendingPurchasesModal({ isOpen, onClose, pendingPurchases, onSelect, onRemove, onClearAll }) {
      ...
      <div className="bg-card text-card-foreground rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[80vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-border">
        <div className="bg-zinc-800 dark:bg-zinc-900 p-5 flex justify-between items-center text-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold tracking-tight">Pending Purchases</h3>
          ...
          <div className="flex items-center gap-2">
            {pendingPurchases.length > 0 && (
              <button 
                onClick={() => {
                  if (confirm('Discard all pending purchases?')) onClearAll();
                }}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {pendingPurchases.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-zinc-400">
              ...
              <p className="text-lg font-semibold">No pending purchases found</p>
              <p className="text-sm">Held purchases will appear here.</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-zinc-500">
                      <span className="flex items-center gap-1">
                        <Clock size={12} /> {new Date(purchase.id).toLocaleTimeString()}
                      </span>
                      <span className="bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">
                        {purchase.rows.filter(r => r.itemCode).length} Items
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1 justify-end text-rose-500 font-black text-lg">
                      ...
                      {Number(purchase.totals.net).toFixed(2)}
                    </div>
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900/30 p-4 border-t border-border flex justify-between items-center text-xs text-zinc-500 font-medium">
          <span>Click Restore to continue a held purchase.</span>
